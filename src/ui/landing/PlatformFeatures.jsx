...
export default function PlatformFeatures() {
    return (
        <section className="px-4 max-w-[1380px] mx-auto pb-16 text-center">
            <h3 className="text-3xl font-bold mb-10 dark:text-slate-50 text-darkViolet">Platform features</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { title: 'Easy items listing', desc: 'Lenders can quickly list their building items with detailed descriptions and photos..', icon: <WrenchScrewdriverIcon className="w-28 h-28 text-gray-600 dark:text-grayishViolet" /> },
                    ...
                    <div key={idx} className="flex bg-white text-darkViolet items-center gap-6 p-5 rounded-md dark:bg-veryDarkViolet shadow-md">
                        {feature.icon}
                        <div className='flex-col flex items-start text-start'>
                            <h4 className="font-bold text-[22px] dark:text-slate-50">{feature.title}</h4>
                            <p className="mt-2 text-xs dark:text-grayishViolet">{feature.desc}</p>
                        </div>
                    </div>
                ))}
            </div>
